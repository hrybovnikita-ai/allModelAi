...
export default function AIPlatform() {
  ...
  return <main className="ai-platform-page">
    ...
    <div className="platform-shell">
      <aside><label><span>Search modules</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tools..." /></label><div>{visible.map(([key,name,description],index)=><button type="button" className={active===key?'active':''} onClick={()=>setActive(key)} key={key}><i>{String(modules.findIndex(([item])=>item===key)+1).padStart(2,'0')}</i><span><strong>{name}</strong><small>{description}</small></span></button>)}</div></aside>
