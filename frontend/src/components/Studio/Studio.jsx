  ...
  return <main className="studio-page"><header className="studio-header"><Link to="/dashboard" className="studio-brand"><span>AI</span>AllModelAI</Link><div><Link to="/chat">Open chat</Link><Link to="/dashboard">Dashboard</Link></div></header>
    ...
    <div className="studio-layout"><nav className="studio-nav">{features.map(([key,label,description])=><button className={active===key?'active':''} onClick={()=>setActive(key)} key={key}><strong>{label}</strong><small>{description}</small></button>)}</nav>
      <section className="studio-panel"><div className="studio-title"><p>WORKSPACE MODULE</p><h2>{title}</h2></div>
        {active==='router'&&<div><p className="studio-copy">Smart Router analyzes intent, language, context size, speed and cost, then explains its choice.</p><div className="mode-grid">{[['quality','Maximum quality'],['balanced','Balanced'],['economy','Save credits'],['speed','Fastest answer']].map(([key,label])=><button className={routerMode===key?'selected':''} onClick={()=>{setRouterMode(key);localStorage.setItem('allmodelai_router_mode',key);setRouteDecision(null)}} key={key}><strong>{label}</strong><small>{key==='quality'?'Best reasoning model':key==='economy'?'Lowest-cost capable model':key==='speed'?'Lowest latency':'Quality and cost together'}</small></button>)}</div><div className="router-preview"><textarea value={routePrompt} onChange={e=>setRoutePrompt(e.target.value)} placeholder="Describe a task to preview routing…"/><button onClick={previewRoute}>Analyze task</button>{routeDecision?.model&&<article><small>{routeDecision.category}</small><strong>{routeDecision.model}</strong><p>{routeDecision.reason}</p></article>}</div><button className="primary-action" onClick={()=>openChat(routePrompt||'Help me with: ')}>Start Smart Chat</button></div>}
        ...
        {dataType&&<><form className="studio-form" onSubmit={addItem}>{active==='document'&&<label className="document-import">Import a text or code file<input type="file" accept=".txt,.md,.csv,.json,.js,.jsx,.ts,.tsx,.py,.html,.css,.pdf,.docx" onChange={importDocument}/></label>}<input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder={active==='assistant'?'Assistant name':active==='prompt'?'Prompt title':active==='memory'?'What should AI remember?':`${title} name`}/>{active!=='memory'&&<textarea value={form.content} onChange={e=>setForm({...form,content:e.target.value})} placeholder={active==='assistant'?'Example: You are a senior product designer…':active==='document'?'Import a file or paste document text here':active==='prompt'?'Prompt text; use {{topic}} for variables':'Project description and knowledge'}/>} {(active==='project'||active==='assistant')&&<input value={form.instructions} onChange={e=>setForm({...form,instructions:e.target.value})} placeholder={active==='assistant'?'Special rules and preferred response style':'Project instructions for AI'}/>}<button>Add {active}</button></form>{active==='document'&&<div className="knowledge-search"><input value={knowledgeQuery} onChange={e=>setKnowledgeQuery(e.target.value)} placeholder="Search across your knowledge base"/><button onClick={searchKnowledge}>Search knowledge</button>{knowledgeResults.map(result=><article key={result.id}><strong>{result.name}</strong><p>{result.excerpt}</p><button onClick={()=>openChat(`Using my knowledge base, answer this question: ${knowledgeQuery}`)}>Ask with sources</button></article>)}</div>}{active==='prompt'&&!items.length&&<div className="template-grid">{promptTemplates.map(([name,prompt])=><button onClick={()=>openChat(prompt)} key={name}><strong>{name}</strong><small>Use template →</small></button>)}</div>}<div className="item-list">{items.map(item=><article key={item.id}><div><strong>{item.name}</strong>{item.content&&<p>{item.content.slice(0,220)}</p>}{item.instructions&&<small>{item.instructions}</small>}</div><span>{active==='prompt'&&<button onClick={()=>openChat(item.content)}>Use</button>}{active==='document'&&<button onClick={()=>openChat(`Using my knowledge base, answer this question: `,'smart')}>Ask</button>}{active==='assistant'&&<button onClick={()=>openChat(`${item.content}\n\nAdditional rules: ${item.instructions}\n\nStart by introducing yourself briefly.`)}>Launch</button>}<button className="danger" onClick={()=>removeItem(item)}>Delete</button></span></article>)}</div></>}
        {active==='team'&&<div className="team-workspace"><form onSubmit={createTeam}><input value={teamName} onChange={e=>setTeamName(e.target.value)} placeholder="New workspace name"/><button>Create workspace</button></form>{teams.map(team=><article key={team.id}><header><div><small>{team.role}</small><h3>{team.name}</h3></div></header><div className="team-members">{team.members.map(member=><span key={member.email}><b>{member.email}</b><small>{member.role}</small></span>)}</div>{team.role!=='viewer'&&<div className="team-invite"><input value={memberEmail} onChange={e=>setMemberEmail(e.target.value)} type="email" placeholder="teammate@example.com"/><button onClick={()=>invite(team)}>Invite editor</button></div>}</article>)}</div>}
