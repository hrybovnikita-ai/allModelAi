...
export default function ChatSettings(){
 ...
 useEffect(()=>{const current=JSON.parse(localStorage.getItem('allmodelai_appearance')||'{}');localStorage.setItem('allmodelai_appearance',JSON.stringify({...current,theme,textColor:color}));document.documentElement.dataset.themePreference=theme;document.documentElement.dataset.theme=theme==='auto'?(window.matchMedia('(prefers-color-scheme: light)').matches?'light':'dark'):theme;document.documentElement.style.setProperty('--user-text-color',color);document.documentElement.style.setProperty('--user-bubble-text',contrast(color));},[theme,color]);
