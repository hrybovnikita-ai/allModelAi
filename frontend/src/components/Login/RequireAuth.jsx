...
export default function RequireAuth() {
  ...
  useEffect(() => {
    ...
    restoreSession()
      .then((user) => {
        if (active) setSession({ status: user ? 'authenticated' : 'anonymous' });
      })
      .catch(() => {
  ...
  if (session.status === 'loading') return <main className="dashboard-page" role="status">Checking your session...</main>;
