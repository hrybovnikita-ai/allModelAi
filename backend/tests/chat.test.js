...
describe('secure chat and knowledge API',()=>{
 ...
 test('falls back from an overloaded direct model to a working gateway model',async()=>{const previousXai=process.env.XAI_API_KEY;process.env.XAI_API_KEY='xai-test-key';const urls=[];global.fetch=async url=>{urls.push(String(url));if(urls.length===1)return new Response(JSON.stringify({error:{message:'This model is currently experiencing high demand'}}),{status:429,headers:{'Content-Type':'application/json'}});return upstream()};try{const response=await api.post('/api/chat').send({model:'grok',temporary:true,messages:[{role:'user',text:'hello'}]});assert.equal(response.status,200);assert.match(response.text,/Secure answer/);assert.match(response.text,/"fallback":true/);assert.match(urls[0],/api\.x\.ai/);assert.match(urls[1],/openrouter\.ai/)}finally{global.fetch=async()=>upstream();if(previousXai===undefined)delete process.env.XAI_API_KEY;else process.env.XAI_API_KEY=previousXai}});
 test('saves generated images in private conversation history', async () => {
   const imageUrl = 'data:image/png;base64,aGVsbG8=';
   const created = await api.post('/api/chat/history').send({ model: 'gpt', messages: [{role:'user',text:'Draw a dragon'}, {role:'assistant',text:'Done',imageUrl}] });
   assert.equal(created.status, 201);
   const history = await api.get('/api/chat/history');
   assert.equal(history.body.find(item => item.id === created.body.id).messages[1].imageUrl, imageUrl);
   assert.equal((await request(app).post('/api/images').send({prompt:'dragon'})).status, 401);
 });
